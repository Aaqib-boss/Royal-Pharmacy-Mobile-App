...
export default function ChequesScreen() {
  ...
  const generateHTML = () => {
    const reportTitle = 'Cheques Clearance Ledger';
    ...
    const rowsHTML = filteredCheques.map((item) => {
      ...
      const chequeBankDetails = `No. ${item.chequeNumber || '-'}<br/><span style="font-size: 9px; color: #64748b;">(${item.bankName || ''})</span>`;
      ...
      return `
        <tr>
          ...
          <td>${chequeBankDetails}</td>
          <td style="color: #0f172a; font-weight: bold;">${amountVal}</td>
        </tr>
      `;
    }).join('');
  ...
  const exportToExcel = async () => {
    ...
    const csvHeaders = ['Date', 'Invoice No', 'Pharmacy Name', 'City', 'Cheque No', 'Bank Name', 'Cheque Amount'];
    const tableHeaders = csvHeaders.map(h => `\"${h}\"`).join(',');

    const tableRows = filteredCheques.map((item) => {
      ...
      const chequeNo = item.chequeNumber || '';
      const bank = item.bankName || '';
      const amountVal = item.amount;

      return [dateStr, invoice, pharmacy, cityVal, chequeNo, bank, amountVal]
        .map(val => `\"${(val || '').toString().replace(/\"/g, '\"\"')}\"`)
        .join(',');
    }).join('\n');

    const csvContent = metadataRows + tableHeaders + '\n' + tableRows;
    const filename = `cheques_report_${Date.now()}.csv`;
  ...
  const fetchData = async () => {
    try {
      const [chequesRes, pharmaciesRes] = await Promise.all([
        api.get('/cheques'),
        api.get('/pharmacies'),
      ]);
      ...
      if (err?.response?.status !== 401) {
        ...
        console.log('Cheques fetch unauthorized (session expired)');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleFormSubmit = async () => {
    ...
    if (!chequeNumber) {
      Alert.alert('Warning', 'Cheque Number is required');
      return;
    }
    if (!bankName) {
      Alert.alert('Warning', 'Bank Name is required');
      return;
    }
    ...
    try {
      if (isEditing && editId) {
        ...
        Alert.alert('Success', 'Cheque entry updated successfully!');
      } else {
        await api.post('/cheques', payload);
        Alert.alert('Success', 'Cheque logged successfully!');
      }
      setShowFormModal(false);
      resetForm();
      fetchData();
    } catch (err: any) {
      Alert.alert('Error', err.response?.data?.message || 'Error saving cheque');
      setLoading(false);
    }
  };
  ...
  const handleDeleteClick = (record: any) => {
    Alert.alert('Are you sure?', `Delete cheque entry for invoice ${record.invoiceNumber}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          setLoading(true);
          try {
            await api.delete(`/cheques/${record._id}`);
            Alert.alert('Success', 'Cheque entry deleted');
            fetchData();
          } catch (error) {
            Alert.alert('Error', 'Failed to remove cheque entry');
            setLoading(false);
          }
        },
      },
  ...
  const filteredCheques = cheques.filter((item) => {
    ...
    if (searchQuery.trim() !== '') {
      ...
      const chqNo = (item.chequeNumber || '').toLowerCase();
    ...
    <SafeAreaView edges={['bottom']} className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      <View className="px-5 py-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex-row justify-between items-center">
        <Text className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">
          {filteredCheques.length} records found
        </Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 shadow-sm mb-4">
            ...
            <View className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-3 mb-4 flex-row items-center">
              <Ionicons name="card-outline" size={16} color="#94a3b8" className="mr-3" />
              <View>
                <Text className="text-slate-800 dark:text-slate-200 text-xs font-bold">
                  No. {item.chequeNumber}
                </Text>
                <Text className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 font-medium">
                  Bank: {item.bankName}
                </Text>
              </View>
            </View>
      ...
      {/* 2. FORM MODAL (ADD / EDIT CHEQUE ENTRY) */}
      <Modal
        ...
        <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950">
          ...
          >
            <Text className="text-base font-bold text-slate-900 dark:text-white">
              {isEditing ? 'Edit Cheque Entry' : 'New Cheque Entry'}
            </Text>
            <TouchableOpacity
              onPress={() => {
                setShowFormModal(false);
            ...
            <View className="space-y-4 pb-10">
              ...
              >
                <Text className="text-white font-bold text-sm">
                  {isEditing ? 'Update Entry' : 'Submit Cheque Entry'}
                </Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
